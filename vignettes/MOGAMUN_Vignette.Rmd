---
title: "MOGAMUN - A Multi-Objective Genetic Algorithm to Find Active Modules in 
Multiplex Biological Networks"
author: 
    -   name: "Elva María Novoa del Toro"
        affilitation: Aix Marseille Univ, INSERM, Marseille Medical Genetics 
        (MMG), Marseille, France
        email: elva.novoa@inserm.fr
        package: MOGAMUN
output: BiocStyle::html_document
vignette: >
    %\VignetteIndexEntry{MOGAMUN - A Multi-Objective Genetic Algorithm to Find 
    Active Modules in Multiplex Biological Networks}
...
overall deregulation) in multiplex biological networks. For a detailed detailed 
...
https://github.com/elvanov/MOGAMUN/PreprintFiles.zip.  
...
`mogamun.init`. In total, there are 11 customizable parameters: 

* `PopSize` is an integer that determines the size of the population 
(*default = 100*).
* `MinNumberOfNodesPerIndividual` and `MaxNumberOfNodesPerIndividual` are 
integers that determine the minimun and maximum number of nodes that are 
allowed in each subnetwork AKA individual, respectively 
...
* `CrossoverRate` is a real value in the range of [0-1] that determines the 
...
* `MutationRate` is a real value in the range of [0-1] that determines the 
...
to determine which statistical value will be taken into account to obtain the 
...
`EvolutionParameters <- mogamun.init()`. Otherwise, specify the parameters to 
...
*MOGAMUN/ExampleFiles.zip* and a desktop computer with Intel processor i7 at 
3.60GHz and 32GB of RAM, the process took approximately 12 hours.
...
data using the `mogamun.load.data` function, which has 5 parameters:
...
the two was seleced in `mogamun.init`) and it should contain the results of the 
...
sample file in *MOGAMUN/ExampleFiles/DE/Sample_DE.csv*.
...
number, which will be used in the `Layers` parameter. There are two example 
files in *MOGAMUN/ExampleFiles/LayersMultiplex*.
* `Layers` is a string composed of a chain of numbers that determine which 
...
dePath <- "/home/elva/MOGAMUN/inst/ExampleFiles/DE/Sample_DE.csv"
scoresPath <- "/home/elva/MOGAMUN/inst/ExampleFiles/DE/Sample_NodesScore.csv"
layersPath <- "/home/elva/MOGAMUN/inst/ExampleFiles/LayersMultiplex/"
loadedData <- 
    mogamun.load.data(
...
ready to run MOGAMUN using the `mogamun.run` function, which has 4 parameters: 
...
processor cores (*default = 1*). One core is needed per run. 
...
subnetwork can vary between `MinNumberOfNodesPerIndividual` and 
`MaxNumberOfNodesPerIndividual`. If **X_n** is the number of elements in the 
...
results, and leave only those subnetworks with rank = 1. For example, in 
Figure 1 we show the results of an experiment of 30 runs with the 
Banerji et al. 2017 dataset that we analyzed in our preprint. In **Figure 1a** 
we show the union of the results of the 30 runs (only those subnetworks with 
rank = 1 in each individual run), re-ranked, and in **Figure 1b** we show the 
resulting accumulated Pareto front. 

![Figure 1. Accumulated Pareto front from 30 runs](/home/elva/MOGAMUN/vignettes/ScatterPlot_AccPF.png)

Depending on the number of runs you execute, we also generate some plots of 
interest, such as scatter plots (always) and boxplots (only if 
`NumberOfRunsToExecute` in `mogamun.run` > 1).



Finally, we visualize the subnetworks in the accumulated Pareto front in 
Cytoscape. To do so, we first filter the networks that were used to build the 
multiplex, to leave only those interactions among the genes that are included 
in the result (i.e. the accumulated Pareto front). Once in Cytoscape, the nodes 
will be colored according to their *logFC* value, where green stands for 
downregulated, red means upregulated, and white means no deregulation. Nodes 
corresponding to significantly differentially expressed genes will have black 
border, and the color of the edges will be different for each layer of the 
multiplex network. 

The postprocessing is done with the function `mogamun.postprocess`, which has 
4 parameters:  
...
Please note that you can make use of `mogamun.postprocess` with any number of 
